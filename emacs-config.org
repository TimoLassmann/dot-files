...
(prot-emacs-elpa-package 'embark
    (let ((map minibuffer-local-map))
    (define-key map (kbd "C-S-a") #'embark-act)
    (define-key map (kbd "C-S-b") #'embark-act))
...
#+RESULTS:
| (lambda nil (add-hook 'before-save-hook 'whitespace-cleanup)) | (lambda nil (set (make-local-variable 'company-backends) '(company-clang company-gtags company-c-headers company-dabbrev))) | tl/setup-c-mode |
